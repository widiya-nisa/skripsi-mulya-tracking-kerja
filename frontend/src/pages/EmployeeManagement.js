...
function EmployeeManagement() {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Apakah Anda yakin ingin menghapus profil karyawan ini?')) {
            return;
        }
        ...
        try {
            ...
            showNotification(error.response?.data?.message || 'Gagal menghapus profil', 'error');
        }
    };
    ...
    const filteredEmployees = employees.filter(emp => {
        ...
        const matchesFilter = filterStatus === 'all' || emp.employment_status === filterStatus;
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">Kelola data profil seluruh karyawan</p>
                    </div>
                            ...
                            {filteredEmployees.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p className="text-sm mt-1">Belum ada karyawan yang terdaftar atau sesuai filter</p>
                                    </td>
                                </tr>
                            ) : (
                                filteredEmployees.map((employee) => (
                                    <tr key={employee.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900 font-medium">{employee.nik || '-'}</div>
                                            <div className="text-sm text-gray-500">{employee.employee_id || '-'}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900 font-medium">{employee.position || '-'}</div>
                                            <div className="text-sm text-gray-500">{employee.join_date || '-'}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-3 py-1 text-xs leading-5 font-semibold rounded-full ${
                                                employee.employment_status === 'permanent' ? 'bg-green-100 text-green-800' :
                                                ...
                                                {employee.employment_status === 'permanent' ? 'Tetap' :
                                                 employee.employment_status === 'contract' ? 'Kontrak' :
                                                 employee.employment_status === 'probation' ? 'Probation' :
                                                 employee.employment_status === 'internship' ? 'Magang' : '-'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-3 py-1 text-xs leading-5 font-semibold rounded-full ${
                                                getUserRole(employee.user_id) === 'admin' ? 'bg-purple-100 text-purple-800' :
                                                getUserRole(employee.user_id) === 'manager' ? 'bg-blue-100 text-blue-800' :
                                                getUserRole(employee.user_id) === 'ceo' ? 'bg-red-100 text-red-800' :
                                                'bg-gray-100 text-gray-800'
                                            }`}>
                                                <svg className="w-3 h-3 mr-1.5" fill="currentColor" viewBox="0 0 20 20">
